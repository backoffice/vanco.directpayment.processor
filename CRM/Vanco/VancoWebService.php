...
class VancoPaymentService
{
    ...
    function __construct( $vanco="ssl://www.vancodev.com", $port=443, $timeout=15 )  {
        ...
        $this->_timeout = $timeout;
  	  	...
  	  	$this->_host 		= substr( $url, $x+3, stripos( $url, '/cgi-bin/' ) - $x-3 );
        $this->_protocol 	= substr( $url, 0, $x+3 );
	...
	function Login($params)
	{
        ...
        $xml_login = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/login.xml");
		...
		$this->_sessionID = ($response_obj) ? (string) $response_obj->Response->SessionID : NULL;
		...
		}else
		{
			...
			$result['error']	= ($response_obj) ? (string) $response_obj->Response->Errors->Error->ErrorCode : NULL;
			$result['desc']		= ($response_obj) ? (string) $response_obj->Response->Errors->Error->ErrorDescription : NULL;
	...
	function Logout()
	{
        $xml_logout = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/logout.xml");
	...
	function EFTAddCompleteTransaction($session_id, $params)
	{
        ...
        $xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/transaction.xml");
	...
	function EFTTransactionHistory($session_id, $params)
	{
        ...
        $xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/history.xml");
	...
	function EFTCurrentTransactions($session_id, $params)
	{
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/recur_history.xml");
	...
	function EFTAddEditPaymentMethod($session_id, $params)
    {
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/addedit.xml");
...

    {
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/getpayment.xml");
        $xml_obj->Auth->RequestTime 	=  date('Y-m-d H:m:s');
		$xml_obj->Auth->SessionID 		= $this->_sessionID;
		foreach( $params as $field => $value )
            {
	...
	function EFTCustomerInfo($session_id, $params)
	{
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/customerinfo.xml");
        $xml_obj->Auth->RequestTime 	=  date('Y-m-d H:m:s');
		$xml_obj->Auth->SessionID 		= $this->_sessionID;
		foreach( $params as $field => $value )
            {
	...
	function EFTDeleteTransaction($session_id, $params)
    {
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/delete.xml");
        $xml_obj->Auth->RequestTime 	=  date('Y-m-d H:m:s');
		$xml_obj->Auth->SessionID 		= $this->_sessionID;
		foreach( $params as $field => $value )
            {
    ...
    function EFTGetFederalHoliday($session_id, $paramsVal)
    {
		...
		$xml_obj   = simplexml_load_file("$this->customExt/vanco.directpayment.processor/CRM/Vanco/xml/federalholiday.xml");
	...
	function log($type,$xml,$fileName = null)
	{
        ...
        $fileName  = $this->customExt."/vanco.directpayment.processor/CRM/Vanco/log/vanco_log_";
        if ($xml ) {
